package com.example.islandbuilder;

import android.content.ContentValues;
import android.content.Context;
import android.database.Cursor;
import android.database.CursorWrapper;
import android.database.sqlite.SQLiteDatabase;
import android.util.Log;

import com.example.islandbuilder.R;

import java.util.Random;

import static android.content.ContentValues.TAG;

/**
 * PROVIDED IN PRACTICAL 3
 * Updated, no longer static, and stores itself in the db
 */
public class MapData
{
    public int WIDTH;
    public int HEIGHT;

    private static final int WATER = R.drawable.ic_water;
    private static final int[] GRASS = {R.drawable.ic_grass1, R.drawable.ic_grass2,
            R.drawable.ic_grass3, R.drawable.ic_grass4};

    private static final Random rng = new Random();
    private MapElement[][] grid;

    private SQLiteDatabase db; //The database where the data is stored

    //Constructor
    public MapData(int width, int height, SQLiteDatabase db, boolean continuing)
    {
        this.db = db;

        if (!continuing)
        {
            generateGrid(width, height);
        } else {
            readDb(width, height);
        }
    }

    //creates the grid randomly, storing each mapElement in the db.
    private void generateGrid(int width, int height)
    {
        //First get rid of any old values in the db
        db.delete(IslandSchema.MapTable.NAME, null, null);

        WIDTH = width;
        HEIGHT = height;
        final int HEIGHT_RANGE = 256;
        final int WATER_LEVEL = 112;
        final int INLAND_BIAS = 24;
        final int AREA_SIZE = 1;
        final int SMOOTHING_ITERATIONS = 2;

        int[][] heightField = new int[HEIGHT][WIDTH];
        for(int i = 0; i < HEIGHT; i++)
        {
            for(int j = 0; j < WIDTH; j++)
            {
                heightField[i][j] =
                    rng.nextInt(HEIGHT_RANGE)
                    + INLAND_BIAS * (
                        Math.min(Math.min(i, j), Math.min(HEIGHT - i - 1, WIDTH - j - 1)) -
                        Math.min(HEIGHT, WIDTH) / 4);
            }
        }

        int[][] newHf = new int[HEIGHT][WIDTH];
        for(int s = 0; s < SMOOTHING_ITERATIONS; s++)
        {
            for(int i = 0; i < HEIGHT; i++)
            {
                for(int j = 0; j < WIDTH; j++)
                {
                    int areaSize = 0;
                    int heightSum = 0;

                    for(int areaI = Math.max(0, i - AREA_SIZE);
                            areaI < Math.min(HEIGHT, i + AREA_SIZE + 1);
                            areaI++)
                    {
                        for(int areaJ = Math.max(0, j - AREA_SIZE);
                                areaJ < Math.min(WIDTH, j + AREA_SIZE + 1);
                                areaJ++)
                        {
                            areaSize++;
                            heightSum += heightField[areaI][areaJ];
                        }
                    }

                    newHf[i][j] = heightSum / areaSize;
                }
            }

            int[][] tmpHf = heightField;
            heightField = newHf;
            newHf = tmpHf;
        }

        grid = new MapElement[HEIGHT][WIDTH];
        for(int i = 0; i < HEIGHT; i++)
        {
            for(int j = 0; j < WIDTH; j++)
            {
                MapElement element;
                int id = i + j * HEIGHT;

                if(heightField[i][j] >= WATER_LEVEL)
                {
                    boolean waterN = (i == 0)          || (heightField[i - 1][j] < WATER_LEVEL);
                    boolean waterE = (j == WIDTH - 1)  || (heightField[i][j + 1] < WATER_LEVEL);
                    boolean waterS = (i == HEIGHT - 1) || (heightField[i + 1][j] < WATER_LEVEL);
                    boolean waterW = (j == 0)          || (heightField[i][j - 1] < WATER_LEVEL);

                    boolean waterNW = (i == 0) ||          (j == 0) ||         (heightField[i - 1][j - 1] < WATER_LEVEL);
                    boolean waterNE = (i == 0) ||          (j == WIDTH - 1) || (heightField[i - 1][j + 1] < WATER_LEVEL);
                    boolean waterSW = (i == HEIGHT - 1) || (j == 0) ||         (heightField[i + 1][j - 1] < WATER_LEVEL);
                    boolean waterSE = (i == HEIGHT - 1) || (j == WIDTH - 1) || (heightField[i + 1][j + 1] < WATER_LEVEL);

                    boolean coast = waterN || waterE || waterS || waterW ||
                                    waterNW || waterNE || waterSW || waterSE;

                    //Changed from !coast to true, since I want to be able to build on the coast just not on the water.
                    element = new MapElement(
                        true,
                        choose(waterN, waterW, waterNW,
                            R.drawable.ic_coast_north, R.drawable.ic_coast_west,
                            R.drawable.ic_coast_northwest, R.drawable.ic_coast_northwest_concave),
                        choose(waterN, waterE, waterNE,
                            R.drawable.ic_coast_north, R.drawable.ic_coast_east,
                            R.drawable.ic_coast_northeast, R.drawable.ic_coast_northeast_concave),
                        choose(waterS, waterW, waterSW,
                            R.drawable.ic_coast_south, R.drawable.ic_coast_west,
                            R.drawable.ic_coast_southwest, R.drawable.ic_coast_southwest_concave),
                        choose(waterS, waterE, waterSE,
                            R.drawable.ic_coast_south, R.drawable.ic_coast_east,
                            R.drawable.ic_coast_southeast, R.drawable.ic_coast_southeast_concave),
                        null, id, db);
                }
                else
                {
                    element = new MapElement(
                        false, WATER, WATER, WATER, WATER, null, id, db);
                }
                grid[i][j] = element;
                ContentValues cv = new ContentValues();

                cv.put(IslandSchema.MapTable.Cols.ID, id);
                if (element.isBuildable()) {
                    cv.put(IslandSchema.MapTable.Cols.BUILDABLE, 1);
                } else {
                    cv.put(IslandSchema.MapTable.Cols.BUILDABLE, 0);
                }
                cv.put(IslandSchema.MapTable.Cols.NORTHEAST, element.getNorthEast());
                cv.put(IslandSchema.MapTable.Cols.NORTHWEST, element.getNorthWest());
                cv.put(IslandSchema.MapTable.Cols.SOUTHEAST, element.getSouthEast());
                cv.put(IslandSchema.MapTable.Cols.SOUTHWEST, element.getSouthWest());

                //Structure is currently null, so set all values as null
                cv.put(IslandSchema.MapTable.Cols.DRAWABLEID, (Integer) null);
                cv.put(IslandSchema.MapTable.Cols.LABEL, (String)null);
                cv.put(IslandSchema.MapTable.Cols.STRUCTNAME, (String)null);

                db.insert(IslandSchema.MapTable.NAME, null, cv);
            }
        }
    }

    private static int choose(boolean nsWater, boolean ewWater, boolean diagWater,
        int nsCoastId, int ewCoastId, int convexCoastId, int concaveCoastId)
    {
        int id;
        if(nsWater)
        {
            if(ewWater)
            {
                id = convexCoastId;
            }
            else
            {
                id = nsCoastId;
            }
        }
        else
        {
            if(ewWater)
            {
                id = ewCoastId;
            }
            else if(diagWater)
            {
                id = concaveCoastId;
            }
            else
            {
                id = GRASS[rng.nextInt(GRASS.length)];
            }
        }
        return id;
    }

    public void regenerate()
    {
        grid = null;
        generateGrid(WIDTH, HEIGHT);
    }

    public MapElement get(int i, int j)
    {
        return grid[i][j];
    }

    //reads each mapElement from the db and stores it in the grid.
    public void readDb(int width, int height){
        CursorWrapper cursor = new CursorWrapper(db.query(IslandSchema.MapTable.NAME, null, null, null, null, null, null));
        cursor.moveToFirst();

        grid = new MapElement[height][width];
        for(int ii = 0; ii < height; ii++) {
            for (int jj = 0; jj < width; jj++) {
                int id = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.ID));
                int buildableInt = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.BUILDABLE));
                boolean buildable = true;
                if (buildableInt == 0) {
                    buildable = false;
                }
                int ne = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.NORTHEAST));
                int nw = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.NORTHWEST));
                int se = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.SOUTHEAST));
                int sw = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.SOUTHWEST));

                Structure struct;
                //If drawableID is null, structure is null
                if (cursor.isNull(cursor.getColumnIndex(IslandSchema.MapTable.Cols.DRAWABLEID))) {
                    struct = null;
                } else {
                    int drawableId = cursor.getInt(cursor.getColumnIndex(IslandSchema.MapTable.Cols.DRAWABLEID));
                    String label = cursor.getString(cursor.getColumnIndex(IslandSchema.MapTable.Cols.LABEL));
                    String structName = cursor.getString(cursor.getColumnIndex(IslandSchema.MapTable.Cols.STRUCTNAME));
                    struct = new Structure(drawableId, label);
                    struct.setName(structName);
                }

                MapElement element = new MapElement(buildable, nw, ne, sw, se, struct, id, db);
                grid[ii][jj] = element;
                cursor.moveToNext();
            }
        }
        cursor.close();
    }



}
