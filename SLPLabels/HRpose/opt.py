...
from  HRpose.utils import utils as ut
...
def parseArgs(if_redef=True):
	...
	# -- env settings
	parser.add_argument('--ds_fd', default='../../SLP2022',
	...
	parser.add_argument('--mod_src', nargs='+', default=['depthRaw'],
	                    help='source modality list, can accept multiple modalities typical model [RGB|IR|depthRaw|PM]') #RGB seems to cause it to fail.
	...
	parser.add_argument('--batch_size_pGPU', default=45, type=int, help='batch size per gpu') 	#45 seems to pretty much max out my 12gb vram for 1 modality. #14 for tri modality

	# test batch size 16 what is the reason,, no idea
		# powers of 2 are optimised in pytorch.
